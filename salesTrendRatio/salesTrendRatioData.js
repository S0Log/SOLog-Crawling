...
const filename = './salesTrendRatio/salesTrendRatioData.json';
const companys = [
...
async function fetchData(url, companyCode) {
    ...
    const salesTrendRatio = [];

    // 테이블 헤더에서 연도 추출
    const years = [];
    $('div.ul_col2_l div.um_table.pd_t1 table thead tr th').each((idx, elem) => {
        const year = $(elem).text().trim();
        if (year && year !== "제품명") {
            years.push(year); // "제품명"은 제외
        }
    });

    // 테이블의 각 행을 순회하며 데이터 추출
    $('div.ul_col2_l div.um_table.pd_t1 table tbody tr').each((idx, elem) => {
        const product = $(elem).find('th div').text().trim(); // 제품명
        if (!product) return; // 빈 데이터 건너뛰기

        // 연도별 데이터를 순회하며 개별 객체 생성
        $(elem).find('td').each((idx, tdelem) => {
            const companyName = code2name[companyCode];
            const salesPercent = $(tdelem).text().trim();
            if (years[idx]) { // 해당 인덱스에 연도가 존재하면 객체 생성
                salesTrendRatio.push({
                    companyName,
                    product,
                    date: years[idx],
                    salesPercent: salesPercent || null, // 값이 없으면 null
                });
            }
        });
    });

    saveToFile(filename, salesTrendRatio);
